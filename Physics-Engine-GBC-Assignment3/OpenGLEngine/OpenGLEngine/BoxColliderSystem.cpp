		...
		for (auto e : getEntities())
		{
			...
			if (boxCollider.body.isAlive() && boxCollider.body.hasComponent<RigidBodyComponent>())
			{
				...
				if (boxCollider.rp3dId < 0)
				{
					...
					body.inertiaTensor[0][0] += (1.0f / 12.0f) * (pow(2 * boxCollider.size.y, 2) + pow(2 * boxCollider.size.z, 2)) / body.reverseQuality;
					body.inertiaTensor[1][1] += (1.0f / 12.0f) * (pow(2 * boxCollider.size.z, 2) + pow(2 * boxCollider.size.x, 2)) / body.reverseQuality;
					body.inertiaTensor[2][2] += (1.0f / 12.0f) * (pow(2 * boxCollider.size.x, 2) + pow(2 * boxCollider.size.y, 2)) / body.reverseQuality;
